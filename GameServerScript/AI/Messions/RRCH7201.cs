...
public class RRCH7201 : AMissionControl
{
	...
	{
		base.OnStartGame();
		base.Game.IsBossWar = "7201";
		if (m_layer == null)
		{
			m_layer = base.Game.Createlayer(1197, 957, "hide", "game.living.Living176", "in", 1, -1);
		}
		for (int i = 0; i < 4; i++)
		{
			redTotalCount++;
			if (i < 1)
			{
				redNpc.Add(base.Game.CreateNpc(redNpcID, 900 + (i + 1) * 100, 900, 1, -1));
			}
			else if (i < 3)
			{
				redNpc.Add(base.Game.CreateNpc(redNpcID, 950 + (i + 1) * 100, 900, 1, -1));
			}
			else
			{
				redNpc.Add(base.Game.CreateNpc(redNpcID, 1000 + (i + 1) * 100, 900, 1, -1));
			}
		}
		blueTotalCount++;
		blueNpc.Add(base.Game.CreateNpc(blueNpcID, 1050, 900, 1, -1));
	}
	...
	public override void OnNewTurnStarted()
	{
		...
		if (redTotalCount < 4 && blueTotalCount < 1)
		{
			for (int i = 0; i < 4; i++)
			{
				...
				if (i < 1)
				{
					redNpc.Add(base.Game.CreateNpc(redNpcID, 900 + (i + 1) * 100, 905, 1, 1));
				}
				else if (i < 3)
				{
					redNpc.Add(base.Game.CreateNpc(redNpcID, 920 + (i + 1) * 100, 905, 1, 1));
				}
				else
				{
					redNpc.Add(base.Game.CreateNpc(redNpcID, 1000 + (i + 1) * 100, 915, 1, 1));
				}
			}
			blueTotalCount++;
			blueNpc.Add(base.Game.CreateNpc(blueNpcID, 1467, 905, 1, 1));
		}
		else
		{
			if (redCount >= 4)
			...
			else if (4 - redCount > 0)
			{
				for (int i = 0; i < 4 - redCount; i++)
				{
					if (redTotalCount < 12 && redCount != 4)
					{
						...
						{
							redNpc.Add(base.Game.CreateNpc(redNpcID, 1000 + (i + 1) * 100, 905, 1, 1));
						}
					}
				}
			}
