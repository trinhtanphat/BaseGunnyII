...
namespace Game.Server.GameObjects
{
    ...
    public class GamePlayer : IGamePlayer
    {
        ...
        private PlayerInventory m_farm;//FARM= 13,
        ...
        {            
            ...
            m_character.hp = (int)((950 + m_character.Grade * 50 + LevelPlusBlood + m_character.Defence / 10 + GetGoldBlood()) * GetBaseBlood());
        }
        public void UpdateBaseCardProperties(int attack, int defence, int agility, int lucky)
        {
            if (attack != m_character.Attack || defence != m_character.Defence || agility != m_character.Agility || lucky != m_character.Luck)
            {
                m_character.Attack = attack;
        ...
        public void UpdateFightPower()
        {
            ...
            FightPower = 0;
            int blood = 0;
            blood = (int)((950 + PlayerCharacter.Grade * 50 + LevelPlusBlood + PlayerCharacter.Defence / 10 + GetGoldBlood()) * GetBaseBlood());
            ...
            {
                //FightPower += (int)(500.0 * Math.Pow(1.2, (double)m_currentSecondWeapon.StrengthenLevel));
            ...
            if (WorldMgr.AddPlayer(m_character.ID, this))
            {
                try
                {
                    if (LoadFromDatabase())
                    {
                        ...
                        Out.SendUpdatePetInfo(this.PlayerCharacter, null);
