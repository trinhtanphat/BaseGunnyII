...
Require ($migration -match "Value='20'\s+WHERE Name='VIPMaxLevel'") 'database VIPMaxLevel migration is missing'
Require ($migration -match "WHERE Name='VIPExpForEachLv'") 'VIP20 cumulative threshold config migration is missing'
Require ($migration -notmatch "Name IN \('VIPExpForEachLv','VIPExpNeededForEachLv'\)") 'legacy VIPExpNeededForEachLv semantics must not be overwritten'
...
Require ($migration -match 'IF @VIPExp < @FloorExp SET @VIPExp=@FloorExp') 'database must enforce the cumulative EXP floor'
