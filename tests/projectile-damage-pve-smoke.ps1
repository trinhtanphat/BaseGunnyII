...
Require (-not $simpleBomb.Contains('return resolvedDamage > 0 ? resolvedDamage : 1;')) 'projectile minimum-damage clamp must not turn exact zero into one'
Require ($simpleBomb.Contains('return damage > 0 && resolvedDamage == 0 ? 1 : resolvedDamage;')) 'only positive fractional projectile damage may be floored to one'
