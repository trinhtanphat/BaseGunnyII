...
$temp = Join-Path $env:TEMP ('bot-terrain-probe-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "terrain probe harness compile failed: $LASTEXITCODE" }
    & $exe
    if ($LASTEXITCODE -ne 0) { throw "terrain probe harness failed: $LASTEXITCODE" }
} finally {
    Remove-Item $temp -Recurse -Force -ErrorAction SilentlyContinue
}
Write-Output 'PVP_BOT_TERRAIN_CLEAR_SMOKE=PASS'
