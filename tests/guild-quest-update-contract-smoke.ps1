...
if ($packet -notmatch '(?s)WriteBoolean\(info\.Data\.IsExist\).*?WriteInt\(3\).*?QuestLevel') { throw 'QUEST_UPDATE must retain the QuestLevel field consumed by the deployed production client.' }
