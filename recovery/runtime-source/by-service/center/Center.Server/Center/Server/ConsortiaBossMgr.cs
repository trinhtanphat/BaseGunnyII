...
public sealed class ConsortiaBossMgr
{
	...
	{
		foreach (ConsortiaInfo value in m_consortias.Values)
		{
			if (value.endTime < DateTime.Now)
			{
				CloseConsortia(value.ConsortiaID, IsBossDie: false);
			}
			if (value.LastOpenBoss.Date < DateTime.Now.Date)
			{
				ResetConsortia(value.ConsortiaID);
			}
		}
	}
	...
	public static void UpdateRank(int consortiaId, int damage, int richer, int honor, string nickName, int UserID)
	{
		...
		try
		{
			if (m_consortias.ContainsKey(consortiaId))
			{
				...
				rankingPersonInfo.ID = m_consortias[consortiaId].RankList.Count + 1;
				...
				m_consortias[consortiaId].RankList.Add(nickName, rankingPersonInfo);
			}
		}
		finally
		{
	...
	public static void UpdateBlood(int consortiaId, int damage)
	{
		...
		try
		{
			if (m_consortias.ContainsKey(consortiaId))
			{
				m_consortias[consortiaId].TotalAllMemberDame += damage;
	...
	public static List<RankingPersonInfo> SelectRank(int consortiaId)
	{
		...
		try
		{
			...
			{
				Dictionary<string, RankingPersonInfo> rankList = m_consortias[consortiaId].RankList;
				IOrderedEnumerable<KeyValuePair<string, RankingPersonInfo>> orderedEnumerable = rankList.OrderByDescending((KeyValuePair<string, RankingPersonInfo> pair) => pair.Value.TotalDamage);
	...
	public static ConsortiaInfo GetConsortiaById(int consortiaId)
	{
		...
		try
		{
			if (m_consortias.ContainsKey(consortiaId))
			{
				result = m_consortias[consortiaId];
			}
		}
		finally
		{
	...
	public static List<int> GetAllConsortiaGetAward()
	{
		...
		try
		{
			foreach (ConsortiaInfo value in m_consortias.Values)
			{
				int consortiaID = value.ConsortiaID;
				if (value.IsSendAward)
				{
