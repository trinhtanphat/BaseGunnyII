...
public class ItemComposeHandler : IPacketHandler
{
	...
	private static readonly double[] composeRate = new double[5] { 0.8, 0.5, 0.3, 0.1, 0.05 };
	...
	public int HandlePacket(GameClient client, GSPacketIn packet)
	{
		...
		if (itemAt2 == null || itemAt == null || itemAt2.Count <= 0)
		{
			client.Out.SendMessage(eMessageType.ERROR, LanguageMgr.GetTranslation("Đặt đá tăng cấp và trang bị cần hợp thành vào!"));
			return 0;
		}
		...
		{
			itemRecordBussiness.PropertyString(itemAt, ref Property);
		}
		...
		{
			flag = flag || itemAt.IsBinds || itemAt2.IsBinds;
			stringBuilder.Append(itemAt.ItemID + ":" + itemAt.TemplateID + "," + itemAt2.ItemID + ":" + itemAt2.TemplateID + ",");
			bool flag3 = false;
			byte b = 1;
			double num3 = composeRate[itemAt2.Template.Quality - 1] * 100.0;
			if (client.Player.StoreBag.GetItemAt(0) != null)
			{
				...
				if (itemInfo != null && itemInfo.Template.CategoryID == 11 && itemInfo.Template.Property1 == 3)
				{
					...
					object obj = text;
					text = string.Concat(obj, "|", itemInfo.ItemID, ":", itemInfo.Template.Name, "|", itemAt2.ItemID, ":", itemAt2.Template.Name);
			...
			if (num2 != -1)
			{
				...
				if (itemInfo2 != null && itemInfo2.Template.CategoryID == 11 && itemInfo2.Template.Property1 == 7)
				{
					...
					stringBuilder.Append(itemInfo2.ItemID + ":" + itemInfo2.TemplateID + ",");
					object obj2 = text;
					text = string.Concat(obj2, ",", itemInfo2.ItemID, ":", itemInfo2.Template.Name);
				}
				else
				{
					itemInfo2 = null;
