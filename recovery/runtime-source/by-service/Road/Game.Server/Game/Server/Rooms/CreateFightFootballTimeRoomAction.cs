...
public class CreateFightFootballTimeRoomAction : IAction
{
	...
	private BaseRoom FindRandomRoom(BaseRoom[] rooms)
	{
		for (int i = 0; i < rooms.Length; i++)
		{
			if (rooms[i] != null && rooms[i].RoomType == m_roomType && !rooms[i].IsPlaying && rooms[i].PlayerCount < 2 && rooms[i].RoomType == eRoomType.FightFootballTime)
			{
				return rooms[i];
			}
		}
