...
public class StreamProcessor
{
	...
	public static byte[] KEY = new byte[8] { 174, 191, 86, 120, 171, 205, 239, 241 };
			...
			lock (tcpQueue.SyncRoot)
			{
				...
				if (tcpQueue.Count > 0)
				{
					do
					{
						...
						int num4 = ((!client.Encryted) ? packetIn.CopyTo(tcpSendBuffer, num, num2) : packetIn.CopyTo3(tcpSendBuffer, num, num2, client.SEND_KEY, ref client.numPacketProcces));
	...
	public void ReceiveBytes(int numBytes)
	{
		...
		try
		{
			...
			do
			{
				...
				{
					int count = receive_fsm.count;
					byte[] param = cloneArrary(m_client.RECEIVE_KEY);
					for (; i + 4 < num; i++)
					{
						byte[] array = decryptBytes(packetBuf, i, 8, param);
						int num3 = (array[0] << 8) + array[1];
						if (num3 == 29099)
						{
				...
				else
				{
					for (; i + 4 < num; i++)
					{
						int num3 = (packetBuf[i] << 8) + packetBuf[i + 1];
						if (num3 == 29099)
						{
				...
				if ((num2 == 0 || num2 >= 20) && num2 <= 8192)
				{
					...
					Array.Copy(packetBuf, i, packetBuf, 0, num4);
					m_client.PacketBufSize = num4;
					break;
				}
				m_client.PacketBufSize = 0;
				if (m_client.Strict)
