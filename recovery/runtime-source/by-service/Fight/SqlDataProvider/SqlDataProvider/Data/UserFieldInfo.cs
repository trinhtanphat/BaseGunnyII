...
public class UserFieldInfo : DataObject
{
	...
	{
		return _payFieldTime == 0 || DateTime.Compare(_payTime.AddDays(_payFieldTime), DateTime.Now) > 0;
	}

	public bool isDig()
	{
		TimeSpan timeSpan = DateTime.Now - _plantTime;
		int num = _fieldValidDate - (int)timeSpan.TotalMinutes;
		return num > 0;
	}
}
